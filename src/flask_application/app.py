...
def create_kafka_consumer():
    ...
    consumer.subscribe(["transport-location"])
    return consumer
...
def sort_routes(route_dict):
    numeric_routes = sorted(
        [route for route in route_dict.keys() if route.isdigit()], key=int
    )
    alphanumeric_routes = sorted(
        [route for route in route_dict.keys() if not route.isdigit()]
    )
...
stops_df["Label"] = (
    stops_df["nazwa_zespolu"].astype(str) + " " + stops_df["slupek"].astype(str)
)


@app.route("/", methods=["GET", "POST"])
def index():
    selected_route = request.args.get(
        "route", "159"
    )  # Default route 159 (it is my favorite route)
    ...
    stops_for_line = stops_for_line.drop(
        columns=["odleglosc", "ulica_id", "typ"]
    ).drop_duplicates()
    stops_for_line = pd.merge(
        stops_for_line,
        stops_df,
    ...
    latest_location_df.rename(
        columns={"Lat": "Latitude", "Lon": "Longitude"}, inplace=True
    )
    latest_location_df["Label"] = (
        latest_location_df["Lines"].astype(str)
        + "/"
        + latest_location_df["Brigade"].astype(str)
    )
    ...
    locations_for_line = latest_location_df[
        latest_location_df["Lines"] == selected_route
    ]
    ...
    return render_template(
        "map.html",
        stops_locations=stops_locations,
